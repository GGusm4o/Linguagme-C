...
void desenharCruz(int tabuleiro[][HAB], int linha_central, int coluna_central, int raio_h, int raio_v) {
    for (int i = linha_central - raio_v; i <= linha_central + raio_v; i++) { // Loop para a linha
        for (int j = coluna_central - raio_h; j <= coluna_central + raio_h; j++) { // Loop para a coluna
            if (linha_central >= 0 && linha_central < HAB && j >= 0 && j < HAB) { // Verifica limites
                tabuleiro[linha_central][j] = 1;
            }
            if (i >= 0 && i < 10 && coluna_central >= 0 && coluna_central < 10) { // Verifica limites
